...
export default class Tree {
  ...
  generateReports(key, people) {
    ...
    branch.reports.forEach(person => this.generateReports(person.key, people));
  }

  addDirectReportsToPerson(person) {
    person.reports = this.people.filter(p => p.boss === person.key);
    if (person.reports.length) {
      person.reports.forEach(report => this.addDirectReportsToPerson(report));
    }
  }
  ...
  generateOrg(key, people) {
    ...
    let anchor = people.find(person => person.key === key.toString());
    ...
    if (anchor.reports.length) {
      anchor.reports.forEach(person => this.addDirectReportsToPerson(person));
    }
  }
